...
#   --repo OWNER/REPO  marketplace repo (default HarshDhussa1008/shipwright; use your org fork)
#
...
TEAM=""
LOCAL=0
REPO="HarshDhussa1008/shipwright"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --local) LOCAL=1; shift ;;
    --repo) REPO="$2"; shift 2 ;;
    --update) echo "--update is no longer needed: run 'claude plugin marketplace update shipwright'"; shift ;;
    *) PROJECT_DIR="$1"; shift ;;
  esac
done
...
echo "  Turn on auto-update once: /plugin -> Marketplaces -> shipwright -> Enable auto-update"
...
if [[ -n "$PROJECT_DIR" ]]; then
  ...
  echo ""
  echo "Open Claude Code in $PROJECT_DIR and run /shipwright:init once more -- it wires the"
  echo "budget statusline (needs the plugin's data dir) and asks for anything it couldn't detect."
else
  echo ""
  echo "Next: open Claude Code in your project and run /shipwright:init"
fi
